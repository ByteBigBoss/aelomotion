        ...
        <div className='flex flex-col w-full '>
            ...
            {SidebarContent.map((data, key) => (
                <div key={key} className='text-start px-4'>
                    <Link href={data.href} onClick={() => setId(data.ID)} className={`w-full text-[16px] line-clamp-2 font-semibold flex items-center gap-2 ${data.ID === id ? 'opacity-100' : 'opacity-80'}`}><Boxes size={18} />{data.name}</Link>
                    <div className='flex flex-col text-[12px] opacity-80 mt-4 '>
                        {data.contents.map((data, key) => (
                            <Link key={key} href={data.href} onClick={() => setId(data.ID)} className={`w-full py-3 box-border px-4 rounded-sm hover:bg-black ${path === data.href && 'bg-black'} flex items-center gap-2`}><CornerDownRight size={18} />{data.name}</Link>
                        ))}
                    </div>
                </div>
            ))}
